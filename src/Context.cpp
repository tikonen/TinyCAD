...
BOOL CContext::SelectFont(LOGFONT &plf, int iRotation)
{
	//int Selected = -1;
	LOGFONT lf = plf;
	...
	if (m_Transform.GetIsYUp()) iRotation = (iRotation >= 2) ? 0 : -900;
	else iRotation = (iRotation >= 2) ? 0 : 900;
	...
	if (IsMetaFile())
	{
		if (Size >= 4)
		{
			...
			for (t = 0.0f; t <= 1.0f; t += 0.02f)
			{
				...
				b = 3 * (t - 2 * t2 + t3);
				c = 3 * (t2 - t3);
				x = a * Np[0].x + b * Np[1].x + c * Np[2].x + t3 * Np[3].x;
				y = a * Np[0].y + b * Np[1].y + c * Np[2].y + t3 * Np[3].y;
...
CSize CContext::GetTextExtentI(CString text)
{
	...
	for (lp = 0; lp != len; lp++)
	{
		if (text[lp] != '`') TextString[lpi++] = text[lp];
	}
...
void CContext::TextOut(CString text, CDPoint da, paint_options options, int dir)
{
	...
	for (lp = 0; lp != lpi; lp++)
	{
		if (BarString[lp] == '_')
		{
			if (dir >= 2) m_pDC->MoveTo(a.x + x_pos, a.y - iHeight);
			else m_pDC->MoveTo(a.x - iHeight, a.y - x_pos);

			x_pos += widths[lp];

			if (dir >= 2) m_pDC->LineTo(a.x + x_pos, a.y - iHeight);
			else m_pDC->LineTo(a.x - iHeight, a.y - x_pos);
		}
		else
		{
			x_pos += widths[lp];
